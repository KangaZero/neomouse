...
/// Accessibility-API window queries. Lets this app observe other apps' focused
/// windows. Requires Accessibility permission in System Settings — without it,
/// all AX calls fail.
public enum Window {
    /// Gets the pixel size of the frontmost app's focused window.
    public static func frontmostAppSize() -> CGSize? {
    ...
    /// Same pattern as frontmostAppSize but fetches both position and size in one go.
    public static func frontmostAppRect() -> CGRect? {
        guard let app = NSWorkspace.shared.frontmostApplication else { return nil }
        return rect(of: app)
    }

    public static func rect(of app: NSRunningApplication) -> CGRect? {
        let axApp = AXUIElementCreateApplication(app.processIdentifier)
