...
// MARK: - Per-mode key handlers
//
// Each handler is the verbatim case body from the old NeoMouseApp.keyHandler
// monolith, with the closure-captured locals (event, appState, asciiKey,
// currentCGPoint, …) routed through `ctx` instead. Internal logic is
// unchanged — splitting this up is a structural refactor, not a behavioural
// one. Sub-refactors (per-keybind helpers inside .normal, registerAction
// table, etc.) are deliberately left for a follow-up.
